...
def _cc_whole_archive_library_impl(ctx):
    ...
    old_linker_inputs = deps_cc_infos.linking_context.linker_inputs.to_list()
    new_linker_inputs = []
    for old_linker_input in old_linker_inputs:
        old_libraries_to_link = old_linker_input.libraries # noqa
        ...
        new_linker_inputs.append(
            cc_common.create_linker_input(
                owner = old_linker_input.owner,
                libraries = depset(new_libraries_to_link),
                user_link_flags = depset(old_linker_input.user_link_flags),
                additional_inputs = depset(old_linker_input.additional_inputs),  # noqa
            )
        )
